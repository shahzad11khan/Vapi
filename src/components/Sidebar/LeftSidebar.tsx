...
const LeftSidebar: React.FC = () => {
  ...
  if (rawToken) {
    token = jwtDecode<DecodedToken>(rawToken);
  }
  console.log(token)
  ...
  return (
    <div className="w-[240px] h-[145vh] bg-[#1C1C1C] p-3 gap-4 flex flex-col shadow-2xl m-5 text-center rounded-2xl">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 cursor-pointer" >
          ...
          <div>
            ...
            <span className="text-white font-medium">{token?.email}</span>
          </div>
          {/* <img
      ...
      <button             
        className={`flex items-center space-x-4 
          h-[48px] rounded-lg px-[15px]  w-[97%] text-left hover:bg-gray-700 hover:border-2 hover:border-gray-500 hover:border-dotted transition-all duration-200 text-white`}
          >
            <img src={'/Vector.png'} alt={"vector"} className="w-6 h-6" />
            <span onClick={handleLogout} className="font-medium text-[15px] leading-[28px] font-open-sans">
              Logout
            </span>
          </button>
